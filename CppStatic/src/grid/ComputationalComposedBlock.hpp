...
namespace Grid {
	...
	class ComputationalComposedBlock: public CommunicativeBlock<ValueFieldIterator<DIMENSIONALITY>, ComposedFieldBoundaryIterator<DIMENSIONALITY>>
	{
	public:
		typedef ValueFieldIterator<DIMENSIONALITY> InnerIteratorType;
		typedef ComposedFieldBoundaryIterator<DIMENSIONALITY> BoundaryIteratorType;
		/**
	...
	: CommunicativeBlock<InnerIteratorType, BoundaryIteratorType>(elementsPerDim) {
		this->extent = extent;
		createGhostRegions();
	...
	: CommunicativeBlock<InnerIteratorType, BoundaryIteratorType>(elementsPerDim, values) {
		this->extent = extent;
		createGhostRegions();
		this->prepareCommunication();
	}
	...
	template <std::size_t DIMENSIONALITY>
	inline FieldIterator<typename ComputationalComposedBlock<DIMENSIONALITY>::BoundaryIteratorType>
	*ComputationalComposedBlock<DIMENSIONALITY>::getBoundaryIterator() const {
		assert(NULL != this->values);
		std::array<std::size_t, DIMENSIONALITY> sizes = this->getSizeArray();
		FieldIterator<typename GhostRegion<DIMENSIONALITY>::BoundaryIteratorType> ***ghostIterators
		= new FieldIterator<typename GhostRegion<DIMENSIONALITY>::BoundaryIteratorType>**[DIMENSIONALITY];
		for (std::size_t i=0; i<DIMENSIONALITY; i++) {
			ghostIterators[i] = new FieldIterator<typename GhostRegion<DIMENSIONALITY>::BoundaryIteratorType>*[2];
			for (std::size_t j=0; j<2; j++) {
				ghostIterators[i][j] = ghostRegions[i][j]->getBoundaryIterator();
			}
		}
		return new BoundaryIteratorType(sizes, &(this->values[this->smallestIndex]), ghostIterators);
	}

	template <std::size_t DIMENSIONALITY>
	inline FieldIterator<typename ComputationalComposedBlock<DIMENSIONALITY>::InnerIteratorType>
	*ComputationalComposedBlock<DIMENSIONALITY>::getInnerIterator() const {
		assert(NULL != this->values);
		std::array<std::size_t, DIMENSIONALITY> sizes = this->getSizeArray();
