...
protected:
	...
	void testGetBoundaryIterator() {
		FieldIterator<BoundaryIteratorType> *iterator = block->getBoundaryIterator();
		// Verify type
	...
	void testGetInnerIterator() {
		FieldIterator<InnerIteratorType> *iterator = block->getInnerIterator();
		// Verify type
	...
	void testReceiveDoneAt() {
		...
		FieldIterator<BoundaryIteratorType> *initializedIterator = block->getBoundaryIterator();
		// Values are the same in all blocks.
		FieldIterator<BoundaryIteratorType> *oppositeIterator = block->getBoundaryIterator();
...
private:
	...
	void verifyGhostRegionValues(ComputationalComposedBlock<DIM>& block) {
		FieldIterator<BoundaryIteratorType> *boundaryIterator = block.getBoundaryIterator();
		FieldIterator<BoundaryIteratorType> *expectedValuesIterator = block.getBoundaryIterator();
		BoundaryId boundary;
		for (std::size_t d=0; d<2*DIM; d++) {
			...
			expectedValuesIterator->setBoundaryToIterate(*oppositeBoundary);
				while (boundaryIterator->isInField()) {
					...
					double expectedValue = expectedValuesIterator->currentNeighbor(boundary.getDimension(), directedNeighborOffset);
					double actualValue = boundaryIterator->currentNeighbor(boundary.getDimension(), directedOffset);
	 ...
	 */
	void verifyInnerValues(ComputationalBlock<InnerIteratorType, BoundaryIteratorType>& block, double *expectedValues) {
		FieldIterator<InnerIteratorType> *iterator = block.getInnerIterator();
		for (std::size_t i=0; i<totalSize; i++) {
