...
namespace Iterators {
	...
	public:
		virtual void SetUp() {
			...
			ghostIterators = new FieldIterator<typename IteratorType::SideIteratorType>**[FIELD_ORDER];
			for (std::size_t i=0; i<FIELD_ORDER; i++) {
				ghostValues[i] = new double*[2];
				ghostIterators[i] = new FieldIterator<typename IteratorType::SideIteratorType>*[2];
				...
				for (std::size_t j=0; j<2; j++) {
					...
					ghostIterators[i][j] = new IteratorType::SideIteratorType(
					ghostSizes, ghostValues[i][j]);
				}
			}
			iterator = new IteratorType(sizes, values, ghostIterators);
		}
	...
	protected:
		typedef ComposedFieldBoundaryIterator<FIELD_ORDER> IteratorType;
		FieldIterator<IteratorType> *iterator;
		std::size_t elementsPerDim;
		std::size_t extent;
		FieldIterator<typename IteratorType::SideIteratorType> ***ghostIterators;
		double ***ghostValues;
		...
		void testCurrentIndex() {
			FieldIteratorTest::testCurrentIndex_boundary(iterator);
		}
		...
		void testFirst() {
			FieldIteratorTest::testFirst_boundary(iterator);
		}

		void testIsInField() {
			FieldIteratorTest::testIsInField_boundary(iterator);
		}
		...
		void verifyCurrentNeighbor(const BoundaryId& boundary) {
			stepIteratorToMiddleOfBoundary_boundary(iterator, boundary);
			...
			for (std::size_t d=0; d<FIELD_ORDER; d++) {
				...
				if (d == boundary.getDimension()) {
					if (boundary.isLowerSide()) {
						...
						expect_equal(expectedValue,
							iterator->currentNeighbor(d, 2));
						neighborIndex[d]-=2;
					} else {
