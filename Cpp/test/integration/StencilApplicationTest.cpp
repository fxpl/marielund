...
public:
	...
	virtual void SetUp(std::size_t pointsPerBlock) {
		...
		inputBlock = new ComputationalComposedBlock<DIM>(pointsPerBlock, ORDER_OF_ACCURACY/2);
		...
		resultBlock = new ComputationalPureBlock<DIM>(pointsPerBlock, resultValues);
		expectedValues = new double[elementsPerBlock];

		stencil = new ConstFD8Stencil<DIM>(stepLength);
	}
...
private:
	...
	BlockOperator<DIM, ORDER_OF_ACCURACY> *stencil;
	double *inputValues;
	double *resultValues;
	double *expectedValues;
	CommunicativeBlock<DIM> *inputBlock;
	ComputationalBlock<DIM> *resultBlock;
